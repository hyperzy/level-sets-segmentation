...
def InitNarrowBand(band_width, rect_ul, rect_br):
	...
	global check_range
	check_range = np.where((psi < band_width) & (psi > 0))
	global inner_range
			...
			for ii, jj in zip(index[0], index[1]):
				if psi[shift_i + ii][shift_j + jj] == 0:
					...
					if (max_dist <= temp):
						...
						max_dist = temp
			ext_phi[i][j] = phi[record_i][record_j]
			ext_phi_gradient[0][i][j] = phi_gradient[0][record_i][record_j]
			ext_phi_gradient[1][i][j] = phi_gradient[1][record_i][record_j]
	return ext_phi, ext_phi_gradient

def ExtensionPhi1(psi, phi, phi_gradient):
	new_psi = InitNarrowBand1(band_width, zero_set)
	sdf_mask = InitSDF(new_psi)
	# new_psi = InitLevelSet(new_psi, sdf_mask)
	ext_phi = np.zeros(img_shape)
	ext_phi_gradient = [np.zeros(img_shape), np.zeros(img_shape)]

	#this is local vairable
	differential_range = np.where(new_psi < band_width)
	for i, j in zip(differential_range[0], differential_range[1]):
		# if it is boundary
		max_dist = 0
		if psi[i][j] == 0:
...
def Evolve(old_psi, phi, phi_gradient):
	...
	delta_t = min(1 / 4 / phi.max(), min(0.5, 4. /(4. + pow(np.fabs(phi_gradient[1]).max() + np.fabs(phi_gradient[0]).max(), 2))))
	iteration = 0

	# check if the sign of differentiable boudary change
	index_diff_boundary = np.where(np.abs(old_psi) == band_width - 1)
	global zero_set
	while not hit_border:
		...
		if fx_by_index[0].size != 0:
			delta[fx_by_index] += normal_force_fac * (ext_phi_dx[fx_by_index] * (psi[(fx_by_index[0], fx_by_index[1] + 1)] - psi[fx_by_index])\
									+ ext_phi_dy[fx_by_index] * (psi[fx_by_index] - psi[(fx_by_index[0] - 1, fx_by_index[1])]))
		if fx_fy_index[0].size != 0:
			delta[fx_fy_index] += normal_force_fac * (ext_phi_dx[fx_fy_index] * (psi[(fx_fy_index[0], fx_fy_index[1] + 1)] - psi[fx_fy_index])\
									+ ext_phi_dy[fx_fy_index] * (psi[(fx_fy_index[0] + 1, fx_fy_index[1])] - psi[fx_fy_index]))
		if bx_by_index[0].size != 0:
			delta[bx_by_index] += normal_force_fac * (ext_phi_dx[bx_by_index] * (psi[bx_by_index] - psi[(bx_by_index[0], bx_by_index[1] - 1)])\
									+ ext_phi_dy[bx_by_index] * (psi[bx_by_index] - psi[(bx_by_index[0] - 1, bx_by_index[1])]))
		if bx_fy_index[0].size != 0:
			delta[bx_fy_index] += normal_force_fac * (ext_phi_dx[bx_fy_index] * (psi[bx_fy_index] - psi[(bx_fy_index[0], bx_fy_index[1] - 1)])\
		...
		if (psi[check_range] * psi_next[check_range] <= 0).any():
			# print(np.where(psi[check_range] * psi_next[check_range] < -1e-8))
			# break
			# if iteration >= 1000:
			# 	break
...
initial_img.in_img = cv2.imread("test1.png")
gray_img = cv2.cvtColor(initial_img.in_img, cv2.COLOR_BGR2GRAY)
# gray_img = cv2.GaussianBlur(gray_img, (0, 0), 1)
...
while (iteration < 500):
	...
	unmasked_psi = InitNarrowBand1(band_width, zero_set)
	# global band_range
	band_range = np.where(unmasked_psi <= band_width)
	# global check_range
	check_range = np.where((unmasked_psi < band_width) & (unmasked_psi > 0))
	# global differential_range
	differential_range = np.where(unmasked_psi < band_width)
